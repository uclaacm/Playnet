...
import {
  getABTestingControlGraph, getABTestingProductGraph, getFinalControlGraph, getFinalProductGraph, numFinalStars,
} from './gameCalculationsUtil';
import { DISPLAY_OPTIONS } from './TimeAllocation';

function ABFinalReport(): JSX.Element {
  const {
    startNewGame, variableSelection, featureWeights, targetWeights, timeAllocation,
            ...
            <div>
              ...
              <div className="bar" style={{ background: getGradient(featureWeights) }}>
                {featureWeights.map((t, i) => (
                  <>
                    <div data-tip data-for={`variable-${i}`} key={i} style={{ width: `${t}%`, height: '30px' }} />
                    <ReactToolTip id={`variable-${i}`}>
                      {t}
                      %
                    </ReactToolTip>
                  </>
            ...
            <div>
              ...
              <div className="bar" style={{ background: getGradient(timePercentages) }}>
                {Object.values(timeAllocation).map((t, i) => (
                  <>
                    <div data-tip data-for={`time-${i}`} key={i} style={{ width: `${timePercentages[i]}%`, height: '30px' }} />
                    <ReactToolTip id={`time-${i}`}>
                      {t}
                      {' '}
                      days
                    </ReactToolTip>
